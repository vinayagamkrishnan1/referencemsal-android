        ...
        btnAction.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (intentData.length() > 0) {
                    if (isEmail)
                        Toast.makeText(getApplicationContext(), "Sending email is not implemented yet", Toast.LENGTH_LONG).show();
                        // startActivity(new Intent(ScannedBarcodeActivity.this, EmailActivity.class).putExtra("email_address", intentData));
                    else {

                        Toast.makeText(getApplicationContext(), "Opening URL in EDGE browser", Toast.LENGTH_LONG).show();
                    ...
                    txtBarcodeValue.post(new Runnable() {
                        ...
                        public void run() {
                            if (barcodes.valueAt(0).email != null) {
                                ...
                                properties.put("Scanned QR Code ", intentData);

                                // Analytics.trackEvent("QR code has been scanned ", properties);
                            } else {
                                isEmail = false;
